...
>
  ...
  >
    {#each Array(9) as _, i}
      <div
        style="position:absolute;left:0;right:0;height:5%;background:linear-gradient(160deg,#0d001a,#1a0040);top:{52 +
          i * 6}%;"
      ></div>
    {/each}
  </div>
  ...
  <!-- horizon grid -->
  <div
    style="position:absolute;bottom:0;left:0;right:0;height:38%;z-index:2;pointer-events:none;
  ...
  <div
    class="hero-text"
    style="flex:1;position:relative;z-index:10;max-width:580px;"
  >
    <p
      bind:this={tagline}
      style="font-size:0.7rem;letter-spacing:0.28em;text-transform:uppercase;color:#ff9f00;margin-bottom:1.5rem;font-weight:700;text-shadow:0 0 16px #ff9f0099;"
    >
      Fullstack Engineer
    </p>
    <h1
      bind:this={heading}
      style="font-size:clamp(3rem,7.5vw,7.5rem);font-weight:900;letter-spacing:-0.03em;line-height:0.95;color:#fff;margin-bottom:2.5rem;text-shadow:0 0 60px #ff4fa344,0 0 120px #9400ff22;"
    >
      {#each words as word}
    ...
    <div bind:this={sub} style="display:flex;flex-direction:column;gap:1.2rem;">
      <p
        style="font-size:0.95rem;color:rgba(255,220,255,0.55);line-height:1.65;max-width:400px;"
      >
        Building mobile & web experiences with React Native, Flutter, and cloud
        infra on GCP & Firebase.
      </p>
      <div style="display:flex;flex-wrap:wrap;gap:0.45rem;margin-top:0.4rem;">
        {#each stack as item}
          <span
            style="font-size:0.68rem;padding:0.28rem 0.85rem;border-radius:2rem;border:1px solid rgba(255,159,0,0.4);color:#ff9f00;letter-spacing:0.07em;font-weight:600;background:rgba(255,159,0,0.08);text-shadow:0 0 8px #ff9f0066;"
          >
            {item}
          </span>
        {/each}
  ...
  <div
    bind:this={scrollCue}
    style="position:absolute;bottom:2rem;left:50%;transform:translateX(-50%);display:flex;flex-direction:column;align-items:center;gap:0.5rem;color:#ff9f0066;font-size:0.65rem;letter-spacing:0.2em;text-transform:uppercase;z-index:10;text-shadow:0 0 10px #ff9f00;"
  >
    <div
      style="width:1px;height:2rem;background:linear-gradient(to bottom,transparent,#ff9f0066);"
    ></div>
    scroll
  </div>
</section>
